...
--
-- Host: localhost
-- Erstellungszeit: 25. Mrz 2019 um 23:28
...
DROP TABLE IF EXISTS Akku;
DROP TABLE IF EXISTS Akkus_zuordnen;
DROP TABLE IF EXISTS Checklisten_elemente;
DROP TABLE IF EXISTS Checkliste_name;
DROP TABLE IF EXISTS Checkliste_zuordnen;
DROP TABLE IF EXISTS ci_sessions;
DROP TABLE IF EXISTS Drohne;
DROP TABLE IF EXISTS Fluege;
DROP TABLE IF EXISTS Fluglog;
DROP TABLE IF EXISTS Flug_zuordnen;
...
--
-- Tabellenstruktur für Tabelle `Fluglog`
--

CREATE TABLE IF NOT EXISTS `Fluglog` (
  `flug_id` int(10) NOT NULL,
  `latitude` varchar(100) NOT NULL,
  `longitude` varchar(100) NOT NULL,
  `altitude` varchar(100) NOT NULL,
  `homeLatitude` varchar(100) NOT NULL,
  `homeLongitude` varchar(100) NOT NULL,
  `battery_percent` varchar(100) NOT NULL,
  `app_warning` varchar(255) NOT NULL,
  PRIMARY KEY (`flug_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

TRUNCATE TABLE `Fluglog`;
-- --------------------------------------------------------
...
ALTER TABLE `Checkliste_werte`
  ADD CONSTRAINT `Checkliste_werte_ibfk_30` FOREIGN KEY (`element_id`) REFERENCES `Checklisten_elemente` (`element_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `Checkliste_werte_ibfk_40` FOREIGN KEY (`flug_id`) REFERENCES `Fluege` (`flug_id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
ALTER TABLE `Email`
  ADD CONSTRAINT `Email_ibfk_1` FOREIGN KEY (`pilot_id`) REFERENCES `Pilot` (`pilot_id`) ON DELETE CASCADE ON UPDATE CASCADE;
